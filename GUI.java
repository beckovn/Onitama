	...
	*/
	...
	public void compTurn() {
		...
		checkWin();
                ...
                /* A Timer is used to introduce a delay of 500 milliseconds (adjust the delay according
                   to your preference) after each move. After the timer fires, it repaints the board,
                   checks for a win, and continues with the next move if it's the computer's turn.
                   This approach ensures that the GUI is updated and repainted after each move, allowing you
                   to see the moves made by the computer players in a computer vs computer game. */
		Timer timer = new Timer(500, e -> {
                    board.repaint(); // Repaint the board after each move
                    checkWin(); // Check for a win after repainting
                    if (gameBoard.isComputerTurn() && !gameBoard.checkWin()) {
                        compTurn(); // Continue the next move if it's the computer's turn
                    }
                });
